    ...
    <div class="row justify-content-center">
        <div class="col-md-6">
            <div class="card card-primary">
                <div class="card-header">
                  <h3 class="card-title">Add New Traffic Point</h3>
                </div>
                @if (session('status'))
                ...
                @endif
                <!-- form start -->
                <form action="{{ route('admin.trafficPoints.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                <div class="card-body">
                    <x-alert />
                  ...
                  <div class="form-group">
                    ...
                    <select class="custom-select form-control-border" id="zone" name="zone_id">
                        <option value="" disabled selected="true">Select thana first</option>
                    </select>
                  </div>
                  <div class="form-group">
                    <label for="name">Traffic Point Name</label>
        ...
        <div class="col-md-6">
            ...
            <div class="card card-success">
                <div class="card-header">
                  <h3 class="card-title success">Traffic Point List</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body p-0">
                  <table class="table table-striped">
                    ...
                    <tbody>
                        @foreach ($trafficPoints as $point)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $point->name }}</td>
                            <td>{{ $point->zone->name ?? '' }}</td>
                            <td>{{ $point->zone->thana->name ?? '' }}</td>
                            <td>{{ $point->zone->thana->district->name ?? '' }}</td>
                            {{-- <td>
                              {{ $point->trafficPoints->count() }}
                            </td> --}}
                            <td></td>
                        </tr>
                        @endforeach
